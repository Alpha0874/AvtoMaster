...
public class DocumentCategoriesFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        new Thread(() -> {
            JsonObject result = PocketBaseClient.getRecords("document_categories");
            if (result != null && result.has("items")) {
                ...
                requireActivity().runOnUiThread(() -> Toast.makeText(getContext(), "Нет категорий документации", Toast.LENGTH_SHORT).show());
            }
        }).start();
    ...
    private void buildMenu(LinearLayout container) {
        ...
        for (int i = 0; i < categoryNames.size(); i++) {
            ...
            card.setOnClickListener(v -> {
                com.avtoforward.automaster.fragments.DocumentModelsFragment fragment = com.avtoforward.automaster.fragments.DocumentModelsFragment.newInstance(categoryId);
